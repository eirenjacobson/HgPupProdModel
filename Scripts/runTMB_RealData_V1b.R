# Function to run TMB Hg Pup Production model on real data
# Last updated 2023-02-07 by EKJ
...
runTMB <- function(data, problist, model){
  
  # PURPOSE
  # Run V1b of the TMB Hg Pup Production model given real data
  # INPUTS
  ...
  # - version: a string, either "V1" or "V2"
  # OUTPUTS
  ...
  TMBdata$nwhite <- data$Whitecoats
  TMBdata$nmoult <- data$Moulted

  # Days on which counts happened (relative to Oct 1st)
  TMBdata$obsdays <- as.numeric(dmy(data$Date) - dmy(paste("0110", unique(data$Year))))
  ...
  lower <- list("N" = max(data$Whitecoats+data$Moulteds), 
              "mubday" = 30, # mubday in days after Oct 1st
              "sdbday" = 1, 
              "alphabday" = 1) # skew parameter

  upper <- list("N" = max(data$Whitecoats+data$Moulteds)*5,
  ...
  start.df <- expand.grid("N" = seq(lower[[1]], upper[[1]], length.out = 10),
                       "mubday" = seq(lower[[2]], upper[[2]], length.out = 10),
                      "sdbday" = seq(lower[[3]], upper[[3]], length.out = 10),
                     "alphabday" = seq(lower[[4]], upper[[4]], length.out = 10))
  ...
  # use random draws to choose 5 values
